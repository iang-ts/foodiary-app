...
export function DateSwitcher() {
  ...
  return (
    <View style={[styles.container, { opacity: isLoading ? 0.5 : 1}]}>
      ...
      <AppText color={theme.colors.gray[700]} weight="medium" style={styles.selecteDate}>
        {formatDate(date)}
      </AppText>

      <Button size="icon" variant="ghost" onPress={nextDay} disabled={isLoading}>
        <ChevronRightIcon />
      </Button>
    </View>
  )
...
function formatDate(date: Date) {
  ...
  return `${isToday ? 'HOJE, ' : ''}${formattedDate.toLocaleUpperCase()}`
}
